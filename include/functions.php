...
function studentindex($username,$cha)
{
	
	$sql = "SELECT FullName, Email, Position, UserName, LastLogin FROM security WHERE UserName = '$username'";
	$query = mysqli_query($cha,$sql) or die("Cannot query the database.<br>" . mysqli_error());
	...
	$result = mysqli_query($cha,$sql) or die("error data.<br>" . mysqli_error());
